...
const ids = require('../../src/config.js').ids;
...
function application() {
  ...
  return new Promise((resolve, reject) => {
    exists(dir, e => {
      ...
      const run = `#!/usr/bin/env bash\n${process.execPath} $(dirname "$0")/host.js`;
      fs.writeFile(path.join(dir, 'run.sh'), run, e => {
        if (e) {
          return reject(e);
        }
        fs.chmodSync(path.join(dir, 'run.sh'), '0755');
        fs.createReadStream('src/host.js').pipe(fs.createWriteStream(path.join(dir, 'host.js')));
        fs.createReadStream('src/messaging.js').pipe(fs.createWriteStream(path.join(dir, 'messaging.js')));
        resolve();
      });
    });
  });
